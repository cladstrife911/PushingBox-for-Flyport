...
HTTP_IO_RESULT HTTPExecuteGet(void)
{
	...
	#ifdef UART_DEBUG
	UARTWrite(1,"## HTTP GET - filename=\"");
	UARTWrite(1,(char*)filename);
	UARTWrite(1,"\"\r\n");
	UARTWrite(1,"## HTTP GET - curHTTP.data=\"");
	UARTWrite(1,(char*)curHTTP.data);
	UARTWrite(1,"\"\r\n");
	#endif
	
	// STEP #2:
	// Handling of the cgi requests, in this case we have only "buttons.cgi" but
	if(!memcmppgm2ram(filename, "buttons.cgi",11))
	{
		//	Now reads the value of the btn request
		//http://192.168.1.115/buttons.cgi?btn=0
	...
	// else if(!memcmppgm2ram(filename, "pushingbox.cgi",14))
	// {
		// // EXEMPLE:
		// // http://192.168.0.115/pushingbox.cgi?v1D89FF15636f4BB&temperature=19.5
		// // curHTTP.data = "v1D89FF15636f4BB&temperature=19.5
		
		// // sendNotification(curHTTP.data,"");
	// }
